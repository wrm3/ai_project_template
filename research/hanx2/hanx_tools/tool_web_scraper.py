...
async def scrape_url_async(url: str, selector: Optional[str] = None) -> str:
    """Scrape a single URL and return its content."""
    results = await process_urls([url], max_concurrent=1)
    if results and len(results) > 0:
        ...
        if selector and content:
            try:
                import re
                # Very simple selector implementation - just look for content between tags
...
def scrape_url(url: str, selector: Optional[str] = None) -> str:
    ...
    Returns:
        str: Scraped content
    """
    return asyncio.run(scrape_url_async(url, selector))
